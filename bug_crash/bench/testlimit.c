...
void main( int argc, char *argv[])
{
	...
	else if( !stricmp( argv[1], "-h")) 
	{
		...
		for(  k=1; k; k++) //无限循环.
		{
			result=OpenEvent( EVENT_ALL_ACCESS, FALSE, "TestLimit" );
			if(!result)
			{
				printf("openEvent 失败 退出\n");
				break;
			}
            i++;
            if( !( i % 100 )) printf("\r%d", i );
