...
string mars::encrypt16(string str, int ed)
{
	...
	if(ed == 1)
	{
		...
		for(int i=0; i<16; i++)
		{
			sprintf(enstr + 2*i, "%02X", educstr[i]);
		}
		//for(int i=0;i<16;i++)
		//{
		//	if(i!=0)
	...
	else
	{
		...
		for(i = 0 ; i < 16 ; i++)
		{
			strncpy((char *)tmp , (char *)(cstr + (i * 2)) , 2);
			sscanf((char *)tmp , "%X" , &j);
			ustr[i] = j;
		}

		decrypt(ustr,educstr);
		return reinterpret_cast<char*>(educstr);
	}
}

string mars::encrypt(string str, int ed)
{
	string enstr,tstr;
	if(ed == 1)
	{
	...
	else
	{
		...
		for(j = 0; j < str.length()/32; j++)
		{
			tstr = str.substr(j * 32, 32);
